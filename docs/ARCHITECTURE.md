...
Shared base settings: Swift 6.0 language mode, deployment target macOS 14.0, `MARKETING_VERSION 1.0.0`, `CURRENT_PROJECT_VERSION 1`, manual signing with `-`, `DEVELOPMENT_TEAM LOCAL_TEAM_ID`, hardened runtime on, dead-code stripping on, user script sandboxing on. There is no entitlements file and no asset catalog in the repository (the `ASSETCATALOG_COMPILER_APPICON_NAME AppIcon` setting is present but nothing supplies it).
...
Signing note: `DEVELOPMENT_TEAM` must be the certificate's OU, `LOCAL_TEAM_ID`. The string in parentheses in the identity name, `LOCAL_CERT_ID`, is not the team and the build fails if it is used. A signed and installed build was confirmed correct by the notification permission prompt appearing from `/Applications`.
...
| Plan said | Code does |
|---|---|
| `DEVELOPMENT_TEAM: LOCAL_CERT_ID` | `LOCAL_TEAM_ID` (the certificate OU) |
