...
The plan below is reproduced from `the original local plan` as approved before the build started. Headings are shifted down one level so the plan nests under this section; the text is otherwise unchanged. Where it disagrees with the code, the code wins and the section above records the difference.
