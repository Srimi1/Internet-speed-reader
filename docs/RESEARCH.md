...
Host used for every VERIFIED line: macOS 26.6.2 (25G83) on Apple Silicon, Xcode 26.6 (17F113), Swift 6.3.3, SDK 26.5, XcodeGen 2.45.4. The client sat in Patna, India, on Bharti Airtel (AS24560). Numbers that depend on the network are that link's numbers, not universal constants.
...
- VERIFIED: Exact JSON shape with the Referer set: `{"hostname","clientIp","httpProtocol":"HTTP/1.1","asn":24560,"asOrganization":"Bharti Airtel Limited","country":"IN","city":"Patna","region":"Bihar","postalCode":"800001","latitude":"25.59408","longitude":"85.13563","colo":{"iata":"CCU","lat":22.654699,"lon":88.446701,"cca2":"IN","region":"Asia Pacific","city":"Kolkata"}}`.
...
- VERIFIED: `GET https://mensura.cdn-apple.com/.well-known/nq` returns `{"version":1,"test_endpoint":"sgsin4-edge-fx-014.aaplimg.com","urls":{"small_download_url":"https://mensura.cdn-apple.com/api/v1/gm/small","large_download_url":"https://mensura.cdn-apple.com/api/v1/gm/large","upload_url":"https://mensura.cdn-apple.com/api/v1/gm/slurp"}}` with response headers `apple-client-asn: 24560` and `apple-client-asn-company: Bharti Airtel Limited`. This is a first-party, key-free ISP name source. The app does not use it today (see the deviations section).
...
- VERIFIED: `security find-identity -v -p codesigning` lists exactly one identity, `Apple Development: 917762034141 (XB7X3VP977)`, and no Developer ID.
- VERIFIED (build session, bug 3): `XB7X3VP977` is the certificate's CN suffix, not the team. `DEVELOPMENT_TEAM` must be the certificate OU, `Z6SX8L9HA7`. `project.yml` carries `DEVELOPMENT_TEAM: Z6SX8L9HA7`; the plan's `XB7X3VP977` was wrong.
...
| Topic | Plan | Code |
|---|---|---|
| `DEVELOPMENT_TEAM` | `XB7X3VP977` | `Z6SX8L9HA7` (the certificate OU; `XB7X3VP977` is the CN suffix). |
