...
| XcodeGen | 2.45.4 |
| Signing identity | `-` |
| Team ID (certificate OU) | `LOCAL_TEAM_ID` |
...
| Network | Example ISP, client in Example City, nearest Cloudflare colo Kolkata (CCU) |
...
3. `DEVELOPMENT_TEAM` was wrong.
   Root cause: the research and the plan assumed the identity's CN suffix `LOCAL_CERT_ID` was the team. The team ID is the certificate's OU, `LOCAL_TEAM_ID`.
   Fix: `project.yml` sets `DEVELOPMENT_TEAM: LOCAL_TEAM_ID`.
...
The plan is `the original local plan`. These are the differences that change what a verifier should expect. The code wins in every case.

| Plan said | Code does | Effect on verification |
|---|---|---|
| `DEVELOPMENT_TEAM: LOCAL_CERT_ID` | `LOCAL_TEAM_ID` | Bug 3 above |
...
| Signature | `codesign --verify --strict --verbose=2 /Applications/InternetSpeedReader.app` | Passes; `codesign -dvv` on the same path shows the Apple Development identity and team `LOCAL_TEAM_ID` |
